...
This script imports daily stock price data and sentiment data from Alpha Vantage
and stores it in BigQuery. It includes robust error handling, data validation,
...
class TimeSeriesImporter:
    ...
    def __init__(self, alpha_client: AlphaVantageClient, bigquery_storage: BigQueryStorage):
        """Initialize the time series importer.
        
        Args:
            alpha_client: Alpha Vantage client
            bigquery_storage: BigQuery storage
        """
        self.alpha_client = alpha_client
        self.bigquery_storage = bigquery_storage
    ...
    def validate_data(self, df: pd.DataFrame, data_type: str) -> tuple[bool, List[str]]:
        ...
        Args:
            ...
            data_type: Type of data (prices, sentiment)
        ...
        if data_type == "prices":
            required_columns = ["date", "symbol", "open", "high", "low", "close", "adjusted_close", "volume"]
            for col in required_columns:
                if col not in df.columns:
                    errors.append(f"Required column '{col}' is missing")
        
        elif data_type == "sentiment":
            required_columns = ["date", "symbol", "time_published", "overall_sentiment_score"]
            for col in required_columns:
                if col not in df.columns:
                    errors.append(f"Required column '{col}' is missing")
        ...
        if data_type == "prices" and "date" in df.columns:
    ...
    def import_ticker_data(self, ticker: str, force_full: bool = False, 
        ...
        try:
            ...
            # Validate price data
            is_valid, errors = self.validate_data(price_df, "prices")
            if not is_valid:
            ...
            if repair_missing or force_full:
                # Get date range from price data
                start_date = price_df["date"].min()
                end_date = price_df["date"].max()
                
                logger.info(f"Fetching sentiment data for {ticker} from {start_date} to {end_date}")
                sentiment_df = self.alpha_client.batch_fetch_sentiment(
                    ticker, 
                ...
                if not sentiment_df.empty:
                    ...
                    is_valid, errors = self.validate_data(sentiment_df, "sentiment")
                    if not is_valid:
                        logger.warning(f"Sentiment data validation failed for {ticker}: {errors}")
                    else:
                        # Store sentiment data in BigQuery
                        logger.info(f"Storing {len(sentiment_df)} sentiment records for {ticker}")
                        sentiment_success = self.bigquery_storage.store_stock_sentiment(ticker, sentiment_df)
                        
                        if not sentiment_success:
                            logger.warning(f"Failed to store sentiment data for {ticker}")
...
def main():
    ...
    try:
        ...
        # Initialize importer
        importer = TimeSeriesImporter(alpha_client, bigquery_storage)
