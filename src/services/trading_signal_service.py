...
from src.models.adaptive_stop_loss import AdaptiveStopLoss
...
class TradingSignalService:
    ...
    def __init__(self):
        ...
        self.stop_optimizer = AdaptiveStopLoss()
    ...
    def train_models(self, historical_data: pd.DataFrame):
        ...
        try:
            ...
            # Train adaptive stop-loss system
            logger.info("Training adaptive stop-loss system...")
            self.stop_optimizer.train(historical_data)
    ...
    def generate_trading_signals(self, current_data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            momentum_score = price_prediction.get('momentum_score', 50)

            # Get optimized stop-loss recommendation
            current_price = current_data['Close'].iloc[-1]
            last_stop = current_price * 0.95  # Default 5% stop

            stop_recommendation = self.stop_optimizer.optimize_stop_loss(
                current_data,
                current_price,
                last_stop,
                momentum_score
            ...
            signals = {
                ...
                'price_forecast': {
                    ...
                    'volume_impact': price_prediction['volume_impact']
                },
                'stop_recommendation': stop_recommendation,
                'risk_analysis': {
                    'current_volatility': float(current_data['Close'].pct_change().std()),
                    'regime_volatility': regime_analysis['regime_volatility'],
                    'volume_profile': self._calculate_volume_profile(current_data),
                    'risk_score': self._calculate_risk_score(
                        current_data['Close'].pct_change().std(),
                        regime_analysis,
                        self._calculate_volume_profile(current_data)
                    )
                }
            }
    ...
    def get_entry_signals(self, data: pd.DataFrame) -> Optional[Dict]:
        ...
        try:
            ...
            # Add momentum impact to reasoning
            ...
            # Add momentum score impact
            momentum_score = price_forecast.get('momentum_score', 50)
            if momentum_score > 70:
                reasoning.append(f"Strong momentum score of {momentum_score}")
                confidence *= 1.2
            elif momentum_score < 30:
                reasoning.append(f"Weak momentum score of {momentum_score}")
                confidence *= 0.8
