#!/usr/bin/env python3
"""
Import script for {TICKER} data.

This script imports time series data for {TICKER} from Alpha Vantage to BigQuery.
It can be used to update the data for the {TICKER} notebook.
"""
import os
import sys
import logging
...
log_file = log_dir / f"import_{TICKER}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Import {TICKER} data from Alpha Vantage to BigQuery")
